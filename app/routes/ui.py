...
@router.post("/ui/ranges/{range_id}/addresses/allocate-next", response_class=HTMLResponse)
async def ui_range_allocate_next_free(
    range_id: int,
    request: Request,
    connection=Depends(get_connection),
    user=Depends(require_ui_editor),
    ...
    try:
        ...
        count = 0
    if count <= 0:
        errors.append("Allocation count must be at least 1.")
    ...
    free_entries = [entry for entry in breakdown["addresses"] if entry["status"] == "free"]
    if count > len(free_entries):
        errors.append(f"Only {len(free_entries)} free addresses are available.")
    ...
    allocated_addresses: list[str] = []
    for entry in free_entries[:count]:
        try:
            repository.create_ip_asset(
                connection,
            ...
            allocated_addresses.append(entry["ip_address"])
        except sqlite3.IntegrityError:
            errors.append("Failed to allocate all requested addresses.")
            break
    ...
    ip_anchor = (allocated_addresses[-1] if allocated_addresses else "").replace(".", "-").replace(":", "-")
    return RedirectResponse(
        url=f"/ui/ranges/{range_id}/addresses#ip-{ip_anchor}",
        status_code=303,
    )
