...
(() => {
  ...
  const validate = () => Boolean(activeTagId) && Boolean(nameInput?.value.trim());
  ...
  editButtons.forEach((button) => {
    button.addEventListener('click', () => {
      setTagFormData({
        ...
        color: button.dataset.tagColor || '#22c55e',
      });
      openDrawer();
    });
  });
