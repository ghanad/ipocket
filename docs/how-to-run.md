...
5) Create Hosts from the **Hosts** page and pick a Vendor when needed. Use the Hosts search panel to filter by text, Vendor, Project, Assignment, linked/free Status, or tags from linked active IPs. Text and select filters update the table immediately with HTMX and no full page reload. Open a Host name from the table to review vendor/status chips and grouped OS, BMC, and other linked IP tables with each IP's project, tags, and notes.
