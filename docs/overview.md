...
- Sidebar includes a **Connectors** page with tabs (`Overview` / `vCenter` / `Prometheus` / `Elasticsearch` / `Cassandra`) so operators can run import connectors directly from UI.
