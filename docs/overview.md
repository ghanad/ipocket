...
- IP assets list includes a responsive multi-action bulk toolbar for selected rows. **Bulk Edit**, **Assign Project**, and **Manage Tags** reuse the existing bulk drawer and mutation path; the overflow is reserved for future destructive bulk actions. The toolbar always shows the cumulative selection count, while the table selection control explicitly scopes **Select this page** and reports both page and total selection counts when selections span pages.
