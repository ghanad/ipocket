...
- Project assignment review is handled directly in the IP Assets list via the **Project Assignment** filter (`All`, `Assigned only`, or `Unassigned only`); the dedicated Needs Assignment page has been removed. The primary filter row is Search, Project, Type, and Project Assignment; archived records remain accessible through direct `archived-only=true` links rather than a visible Status filter.
...
- Data-table rows use one shared compact action pattern across IP Assets, Hosts, Ranges, Library catalogs, and User Management. The Actions header and reserved cells align to the logical right/end edge consistently, but remain visually empty until the row is hovered or receives keyboard focus; **Edit** and the `⋯` trigger then appear as matching lightweight ghost controls with a subtle 170 ms fade/slide transition. Individual controls receive a stronger background only on hover/focus. Touch/coarse-pointer and small-screen layouts keep the controls visible because hover is unavailable. **Delete** remains the final destructive item in the overflow menu. The menu is portal-positioned to avoid table clipping, follows RTL/LTR direction, supports keyboard navigation and Escape/outside-click close, and leaves room for future non-destructive actions.
...
- The authenticated `/ui/about` page is React/Vite/TypeScript-powered inside the existing application shell. It loads only safe version, commit, build-time, and operational-link data from `GET /api/ui/about`; `/health` and `/metrics` remain unchanged direct operational endpoints.
...
- IP assets list includes a responsive multi-action bulk toolbar for selected rows. **Bulk Edit**, **Assign Project**, and **Manage Tags** reuse the existing bulk drawer and mutation path; a row-style `⋯` overflow is reserved for future destructive bulk actions. The toolbar is the single selection-status summary and always shows the cumulative count, while the tri-state table checkbox is explicitly labeled **Select all on this page**.
- IP assets table headers use higher-contrast, medium-light text for easier scanning without increasing their size or spacing.
...
- Legacy Range Addresses `#used` and `#free` links are canonicalized on load to `?status=used` and `?status=free`; the obsolete fragment is removed so later filter changes cannot silently reapply it.
...
- Sidebar includes a React-based **Connectors** page with tabs (`Overview` / `vCenter` / `Prometheus` / `Elasticsearch` / `Cassandra` / `Ceph` / `Kubernetes`) so operators can run import connectors directly from UI. Canonical `?tab=...` URLs use browser history, and `?tab=...&job_id=...` restores polling/results after reload while the in-memory job remains available. Temporary polling failures can be retried without changing that URL; expired jobs must be dismissed and re-run. Connector jobs are process-local and expire after the one-hour retention window or a server restart.
...
- Connector UI routes are split under `app/routes/ui/connector_routes/`: each connector retains its execution worker, background-job wrapper, and legacy POST route; shared React API schemas/validation, page mounting, job storage, and Prometheus preview formatting live in focused support modules. `app/routes/ui/connectors.py` remains the stable router-composition facade.
