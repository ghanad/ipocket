...
- Range addresses view includes IP-assets-style HTMX filters (search + project + type) so you can narrow results by project and type together.
- Range addresses table supports click-to-filter chips: clicking Project/Type sets those filters, and Tag chips toggle the same multi-tag filter model used in IP assets.
