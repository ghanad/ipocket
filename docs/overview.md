...
- Sidebar includes a **Connectors** page with tabs (`Overview` / `vCenter` / `Prometheus` / `Elasticsearch` / `Cassandra` / `Ceph`) so operators can run import connectors directly from UI.
