...
- Tags page now uses the same right-side drawer pattern as Projects/Vendors for create/edit/delete, including destructive confirmation by typing the exact tag name. Drawer actions also expose direct links (`/ui/tags?create=1`, `/ui/tags/{id}/edit`, `/ui/tags/{id}/delete`) so controls still work without JavaScript.
