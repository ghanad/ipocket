...
Phase 5 moves Library and IP Asset Detail to the shared transport. Library keeps
its encoded full current-location authentication return target, including the
pathname and query string. IP Asset Detail keeps its pathname-only return target
and continues exposing authentication as page-domain status `303`. Library CRUD
payloads, IP Asset Detail's update transformation, and both pages' permanent
